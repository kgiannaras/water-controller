...
lcd.setCursor(0,0);
 ...
 if((soilMoisture < 30) && (tankfill1> 11)){
  lcd.clear();
...
if (digitalRead(Relay3) == LOW){
  Serial.println("passed if relay3  low");
  if((soilMoisture > 50) || (tankfill1 < 11)){
    digitalWrite(Relay3,HIGH);
   digitalWrite(led3,LOW);
   Serial.println("passed if tank1 low and moisture high");
  }
}
